...
function montarCardsConsultas(){
    ...
    for(var i = 0; i < listaConsultas.length; i++){
        ...
        conteudo += '</div>';
        conteudo += '<div class="divCardEndereco">'
        conteudo += listaConsultas[i][4]
        conteudo += '</div>'
        conteudo += '<div class="divCardPreco">';
        conteudo += listaConsultas[i][5];            
        conteudo += '</div>';
        if(listaConsultas[i][6] == false){
...
function CriarConsulta(){
    ...
    if(endereco.length < 10){
        document.getElementById("divEndereco").classList.add('inputErro');
    }else{
        document.getElementById("divEndereco").classList.remove('inputErro');
    }
    if(preco.includes("R$")){
    ...
    if(nome.length > 5 && email.includes("@") && horario.length == 16 && preco.includes("R$") && endereco.length >= 10){
        listaConsultas.push([x, nome, email, horario, endereco, preco, false]);
        montarCardsConsultas(); 
        x++;
